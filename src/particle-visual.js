...
function loadImage() {
  ...
  img.onload = () => {
    ...
    // Calculate aspect ratio and dimensions to fit within 's' while preserving ratio
    ...
    // Center the image
    const ox = (PV.config.logoSize - dw) / 2;
    const oy = (PV.config.logoSize - dh) / 2;
...
function createParticles(pixels) {
  ...
  const scale = Math.min(innerWidth / 1920, 1) * 1.2;
  const dim = PV.config.logoSize;
  const spacing = PV.config.particleSpacing;
  const darken = 0.4; // Darken factor (lower is darker)
